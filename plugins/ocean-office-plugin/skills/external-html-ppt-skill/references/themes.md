...
每个主题是 `assets/themes/` 下的一个简短 CSS 文件，覆盖 `assets/base.css` 中定义的令牌。通过更改 `<link id="theme-link">` 的 `href` 来切换主题，或者如果演示文稿在 `<body>` 或 `<html>` 上有 `data-themes="a,b,c"` 属性，按 **T** 键切换。

所有主题定义相同的变量：`--bg`、`--bg-soft`、`--surface`、`--surface-2`、`--border`、`--text-1/2/3`、`--accent`、`--accent-2/3`、`--good`、`--warn`、`--bad`、`--grad`、`--grad-soft`、`--radius*`、`--shadow*`、`--font-sans`、`--font-display`。
...
复制一个现有主题，重命名，仅覆盖你想要更改的变量。每个主题保持在约 200 行以内。优先调整令牌而非添加新的选择器。
